...
namespace Lab13
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (index < s.Length)
            {
                if (radioButton1.Checked && value[index, 0] == value[index, sum[index]])
                    result++;
                else if (radioButton2.Checked && value[index, 1] == value[index, sum[index]])
                    result++;
                else if (radioButton3.Checked && value[index, 2] == value[index, sum[index]])
                    result++;
                else if (radioButton4.Checked && value[index, 3] == value[index, sum[index]])
                    result++;
                ...
                if (index == s.Length)
                {
                    MessageBox.Show($"Тест завершен! Ваш результат: {result}/{s.Length}");
                    button2.Enabled = false;
                }
            }
            else
            {
                MessageBox.Show("Выберите ответ, прежде чем перейти к следующему вопросу.");
            }
        }
